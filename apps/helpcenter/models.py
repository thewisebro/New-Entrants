...
class Response(models.Model):
    ...
    if not Notification.filter(app='helpcenter', instance=self).exists():
      if flag:
        Notification.save_notification('helpcenter', "A user "+escape(self.text[0].lower()+self.text[1:]).replace('\n','<br>'),
            '/helpcenter/queries/#'+str(self.pk), Group.objects.get(name='IMG Member').user_set.all(), self)
      else:
        Notification.save_notification('helpcenter', self.user.html_name+(' gave a feedback: ' if\
            ...
            Group.objects.get(name='IMG Member').user_set.all(), self)
    return result
  ...
  def last_replied_by_IMG(self):
    ...
    return True if u.in_group('IMG Member') else False
...
class Reply(models.Model):
  ...
  def clean(self,**kwargs):
    if not (self.user == self.response.user or self.user.in_group('IMG Member')):
  ...
  def save(self, *args, **kwargs):
    ...
    self.by_img = self.user.in_group('IMG Member')
    result = super(Reply, self).save(*args, **kwargs)
    if self.user.in_group('IMG Member'):
