...
&nbsp;    
    &nbsp; <a href="logout.php" style="float:right;">Logout</a>
      ...
      foreach($partC_options as $key=>$value){
			...
			echo "</div><br/><br/>";
      $id=$_SESSION['id'];
			$query = "SELECT * FROM questions WHERE id>=30 and id<=39";
      ...
      echo "</table><br/>";
      $query="SELECT * FROM questions WHERE number='C_others'";
      $result=mysql_query($query, $con);
      $result=mysql_fetch_assoc($result);
      $name=$result['question'];
      $query="SELECT * FROM survey WHERE person_id='$id' and question='C_others'";
      ...
      if($answer==''){
        echo "<span><textarea name='C_others'></textarea></span><br/>";
			}
      else{
         echo "<span><textarea name='C_others'>$answer</textarea></span><br/>";	     
      }
      ?>
      <br/>
      <div style="width:50%; margin:0 auto;">
      <span style='margin:auto;'>
				<input class="btn-primary" type="button" value="Previous" onClick="window.location.href='survey_partC1.php'">	
			</span>	&nbsp; &nbsp; &nbsp;
			<span style='margin:auto;'><input class="btn-primary" type='submit' name='partC_submit' value='Next'></span>
		  </div>
    </form>
	 </div>
   </div>
