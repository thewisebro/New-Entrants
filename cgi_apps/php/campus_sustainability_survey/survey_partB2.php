...
&nbsp; &nbsp; <a href="logout.php" style="float:right;">Logout</a>
      ...
      foreach($partB_options as $key=>$value){
			...
			echo "</div><br/><br/>";
  		$id=$_SESSION['id'];
			$query = "SELECT * FROM questions WHERE id>=10 and id<=19";
      ...
      echo "</table><br/>";
      $query="SELECT * FROM questions WHERE number='B_others'";
      $result=mysql_query($query, $con);
      $result=mysql_fetch_assoc($result);
      $name=$result['question'];
      $query="SELECT * FROM survey WHERE person_id='$id' and question='B_others'";
      ...
      if($answer==''){
        echo "<span><textarea name='B_others'></textarea></span><br/>";
			}
      else{
         echo "<span><textarea name='B_others'>$answer</textarea></span><br/>";	     
      }
      ?>
		  <br/>
      <div style="width:50%; margin:0 auto;">
        <span style=''>
  				<input type="button" class="btn-primary" value="Previous" onClick="window.location.href='survey_partB1.php'">	
  			</span>
        &nbsp; &nbsp; &nbsp;   
  			<span style=''><input class="btn-primary" type='submit' name='partB_submit' value='Next'></span>
		  </div>
     </form>
     </div>
	  </div>
