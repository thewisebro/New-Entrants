...
&nbsp;  &nbsp; <a href="logout.php" style="float:right;">Logout</a>
		<h3>Part C. SUSTAINABILITY PRACTICES WITHIN THE CAMPUS</h3>
		<br/>
	<div class='question_1'>	<b>Q.1. Based on the above sustainability parameters add your experience/observation within your campus(after joining the campus and the present day scenario). Please tick the option mentioned below in an appropriate columns as far as <u>the status of the activity being carried out within the campus</b></u></div>
    <br/>
		...
		<?php
			...
			$count_group=0;
			$count_alphabet=-1;
      echo "<div class='partC_options'>";
      foreach($partC_options as $key=>$value){
				echo " <span class='partC_option'> &nbsp; <i>$key.$value</i> &nbsp; </span>";	
			}
			echo "</div><br/><br/>";
  		$id=$_SESSION['id'];
			$query = "SELECT * FROM questions WHERE id>=21 and id<=29";
			$rlt = mysql_query($query, $con) or die("Error in connection. Please try again.");
			$count=0;
			echo "<table><tr><td></td>";
     echo "<td>&nbsp; 1.</td><td>&nbsp; 2.</td><td>&nbsp; 3.</td><td>&nbsp; 4.</td><td>&nbsp; 5.</td></tr>";
	      ...
	      $query="SELECT * FROM survey WHERE person_id='$id' and question='$name'";
	  		...
	  		$result=mysql_fetch_assoc($result);
				...
				$count_roman++;
				echo "<tr><td width='600px' height='40px' id='$name'>($roman[$count_roman]) $row[question]</td>";
				$prev=explode('_',$row['number']);
				foreach($partC_options as $key=>$option){
					if($option == $answer){
      ...
      <br/>
        ...
        <span> <input class="btn-primary" type="button" value="Previous" onClick="window.location.href='survey_partB2.php'">	</span>
			  &nbsp; &nbsp; &nbsp; 
			  <span> <td><input class="btn-primary" type='submit' name='partC_submit' value='Next'></td></tr></span>
		  </div>
		</form>
    </div>
   </div>
