 ...
 <head>
  ...
  <div class="container">
    ...
    <?php 
      ...
      ?>
&nbsp; &nbsp; <a href="logout.php" style="float:right;">Logout</a></div>
		<h3>Part B. SUSTAINABILITY PERCEPTION</h3>
		<br/>
	  <div class='question_1'><b>	Q.1. How important are the factors below to improve  the sustainability of your campus? (Please select the appropriate choice)</b></div>
    <br/>
		...
		<?php
			...
			$count_group=0;
			$count_alphabet=-1;
			echo "<div class='partB_options'>";
      foreach($partB_options as $key=>$value){
				echo " <span class='partB_option'> &nbsp; <i>$key.$value</i> &nbsp; </span>";	
			}
			echo "</div><br/><br/>";
  		$id=$_SESSION['id'];
			$query = "SELECT * FROM questions WHERE id>=1 and id<=9";
      ...
      while($row = mysql_fetch_assoc($rlt)){ 
        ...
        $query="SELECT * FROM survey WHERE person_id='$id' and question='$name'";
	  		...
	  		$result=mysql_fetch_assoc($result);
				...
				foreach($partB_options as $key=>$option){
					if($option == $answer){
						echo '<td class="'.$name.'" width="75px"> &nbsp; <input type="radio" name="'.$name.'" value="'.$option.'" checked="checked"/>&nbsp; </td>';	
	        }
					else{
      ...
      <div style="width:50%; margin:0 auto;">	
			  &nbsp; &nbsp; &nbsp; 
			  <span> <td><input type='submit' name='partB_submit' class="btn-primary" value='Next'></td></tr></span>
		  </div>
    </form>
    </div>
    </div>
